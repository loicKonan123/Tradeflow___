...
public record StartProjectWorkCommand(Guid ProjectId) : IRequest<Result>;

public class StartProjectWorkCommandHandler(IApplicationDbContext db) : IRequestHandler<StartProjectWorkCommand, Result>
{
    public async Task<Result> Handle(StartProjectWorkCommand request, CancellationToken ct)
    {
